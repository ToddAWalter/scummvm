...
AlsaDevices AlsaMusicPlugin::getAlsaDevices() const {
	...
	while (snd_seq_query_next_client(seq_handle, cinfo) >= 0) {
		...
		snd_seq_port_info_set_port(pinfo, -1);
		// removed found_valid_port as it's preventing enumerating all the ports; get info return being negative will break the loop instead
		while (snd_seq_query_next_port(seq_handle, pinfo) >= 0) {
			if (check_permission(pinfo)) {
				// added (pinfo) below to get port name to differentiate between devices on a multiplexer
